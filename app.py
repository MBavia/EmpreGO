...
def download(id_candidato):
    try:
        conexao, cursor = conectar_db()
        comandoSQL = "SELECT * FROM candidato WHERE id_candidato = %s"
        cursor.execute(comandoSQL, (id_candidato,))
        candidato = cursor.fetchone()
        return send_from_directory(app.config['UPLOAD_FOLDER'], candidato['curriculo'], as_attachment=False)
    except Error as erro:
        return f"ERRO! Erro de Banco de Dados: {erro}"
    except Exception as erro:
        return f"ERRO! Outros erros: {erro}"
...
@app.route('/delete/<int:id_candidato>')
def delete(id_candidato):
    try:
        ...
        cursor.execute(comandoSQL, (id_candidato,))
        candidato = cursor.fetchone()
        os.remove(os.path.join(app.config['UPLOAD_FOLDER'], candidato['curriculo']))
        comandoSQL = "DELETE FROM candidato WHERE id_candidato = %s"
        cursor.execute(comandoSQL, (id_candidato,))
        conexao.commit()
        return redirect(f'/candidatos/{candidato['id_Vaga']}')
...
def buscar_vagas():
    ...
    try:
        ...
        return f"ERRO! Erro de Banco de Dados do sistema: {erro}"
    except Exception as erro:
        return f"ERRO! Outros erros: {erro}"
    finally:
        encerrar_db(cursor, conexao)
